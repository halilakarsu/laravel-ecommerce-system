                    ...
                    <div class="col-lg-8 p-r-0 title-margin-right">
                        <div class="page-header">
                            <div class="page-title">
                                <h1>Soru Cevap Sayfası <br><span>Siteye ait soru cevap bölümüne hoş geldiniz.</span>
                                </h1>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="page-header">
                            <div class="page-title">
                                <ol class="breadcrumb">
                                    ...
                                    <a href="{{route('questions.create')}}" class="btn btn-success mini mt-5"><i class="fa fa fa-plus-circle"></i><small> Ekle</small></a>
                                </ol>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="bootstrap-data-table-panel">
                                    <div class="table-responsive">
                                        <table id="example" class="display" style="width:100%">
                                            ...
                                            <tr id="item-{{$key->id}}">
                                                <td class="sortable" ><img width="70px" src="/backend/images/questions/{{$key->question_imagepath}}" alt=""></td>
                                                <td>{{$key->question_title}}</td>
                                                <td>  <div style="margin-left:-40px;margin-top:10px" class="form-check form-switch text-lg-left ">
                                                                   <label class="custom-switch">
                                                                <input data-id="{{$key->id}}" type="checkbox" class="custom-switch-input" {{$key->question_status==1 ? "checked": ""}}>
                                                                <span class="custom-switch-slider"></span>
                                                            </label>
                                                    </div>
                                                   </td>
                                                <td>
                                                    <a title="Düzenle" class="button btn-success btn mini" href="{{route('questions.edit',$key->id)}}">  <i class="fa fa-edit"></i></a>
...
<script>
 ...
 $(document).on('click', '.btn-danger', function() {
     var questionId = $(this).data('id'); // Tıklanan butondan question ID'yi al
     var $row = $(this).closest('tr'); // Silinen questionun satırını seç
         alertify.confirm('Lütfen Silme İşlemini Onaylayın','Bu işlem bir daha geri alınmayacaktır',
             function () {
                 $.ajax({
                     type:"DELETE",
                     url: '/questions/' + questionId,  // DELETE isteği için URL
