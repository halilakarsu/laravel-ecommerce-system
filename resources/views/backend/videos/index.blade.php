                    ...
                    <div class="col-lg-8 p-r-0 title-margin-right">
                        <div class="page-header">
                            <div class="page-title">
                                <h1>Video Sayfası <br><span>Siteye ait video detayları bu sayfada düzenlenmektedir.</span>
                                </h1>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="page-header">
                            <div class="page-title">
                                <ol class="breadcrumb">
                                    ...
                                    <a href="{{route('videos.create')}}" class="btn btn-success mini mt-5"><i class="fa fa fa-plus-circle"></i><small> Ekle</small></a>
                                </ol>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="bootstrap-data-table-panel">
                                    <div class="table-responsive">
                                        <table id="example" class="display" style="width:100%">
                                            ...
                                            <tr id="item-{{$key->id}}">
                                                <td class="sortable" ><img width="70px" src="/backend/images/videos/{{$key->video_imagepath}}" alt=""></td>
                                                <td>{{$key->video_title}}</td>
                                                <td>  <div style="margin-left:-40px;margin-top:10px" class="form-check form-switch text-lg-left ">
                                                                   <label class="custom-switch">
                                                                <input data-id="{{$key->id}}" type="checkbox" class="custom-switch-input" {{$key->video_status==1 ? "checked": ""}}>
                                                                <span class="custom-switch-slider"></span>
                                                            </label>
                                                    </div>
                                                   </td>
                                                <td>
                                                    <a title="Düzenle" class="button btn-success btn mini" href="{{route('videos.edit',$key->id)}}">  <i class="fa fa-edit"></i></a>
...
<script>
 ...
 $(document).on('click', '.btn-danger', function() {
     var videoId = $(this).data('id'); // Tıklanan butondan video ID'yi al
     var $row = $(this).closest('tr'); // Silinen videoun satırını seç
         alertify.confirm('Lütfen Silme İşlemini Onaylayın','Bu işlem bir daha geri alınmayacaktır',
             function () {
                 $.ajax({
                     type:"DELETE",
                     url: '/videos/' + videoId,  // DELETE isteği için URL
