                    ...
                    <div class="col-lg-8 p-r-0 title-margin-right">
                        <div class="page-header">
                            <div class="page-title">
                                <h1>Ürün Sayfası <br><span>Siteye ait ürün detayları bu sayfada düzenlenmektedir.</span>
                                </h1>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div class="table-responsive">
                                        <table id="example" class="display" style="width:100%">
                                            ...
                                            @foreach($products as $key)
                                                <tr id="item-{{$key->id}}">
                                                    <td class="sortable" ><img width="70px" src="/backend/images/products/{{$key->product_imagepath}}" alt=""></td>
                                                    <td>{{$key->product_title}}</td>
                                                    <td>  <div style="margin-left:-40px;margin-top:10px" class="form-check form-switch text-lg-left ">
                                                            <label class="custom-switch">
                                                                <input data-id="{{$key->id}}" type="checkbox" class="custom-switch-input" {{$key->product_status==1 ? "checked": ""}}>
                  ...
                  <script>
                            ...
                            $(document).ready(function() {
                                ...
                                $(document).on('click', '.btn-danger', function() {
                                    ...
                                    var $row = $(this).closest('tr'); // Silinen productun satırını seç
                                    alertify.confirm('Lütfen Silme İşlemini Onaylayın','Bu işlem bir daha geri alınmayacaktır',
                                        function () {
                                            $.ajax({
                                                type:"DELETE",
                                                url: '/products/' + productId,  // DELETE isteği için URL
